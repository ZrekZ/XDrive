...
extern unsigned char i2cReceiveArr[2], i2cSendArr[4];
...
{
	Dma1ChannelOnOffState(6,0); // Включаем ДМА на передачу
	setDma1DataWidth(6,2);	// Кол-во данных передаваемых по ДМА
	Dma1ChannelOnOffState(6,1); // Включаем ДМА на передачу
...
{
	Dma1ChannelOnOffState(6,0); // Включаем ДМА на передачу
	setDma1DataWidth(6,4);   // Кол-во данных передаваемых по ДМА
	Dma1ChannelOnOffState(6,1); // Включаем ДМА на передачу
	I2C2->CR2&=0xFFFFFBFF;	// Включаем режим записи
	I2C2->CR2&=0xFF00FFFF;	// Обнуляем кол-во считываемых байт
...
{
	Dma1ChannelOnOffState(7,0); // Включаем ДМА на передачу
	setDma1DataWidth(7,2);   // Кол-во данных передаваемых по ДМА
	Dma1ChannelOnOffState(7,1); // Включаем ДМА на передачу
...
unsigned char ReadParamsFromMem (unsigned char firstParNum, unsigned char numOfParams) // Чтение из памяти последовательности параметров
{
	...
	else if (stage==1)
	{
		if (n!=numOfParams)
		{
			...
			if (status==1)
			{
				SaveParam2Var(index,(i2cReceiveArr[0]<<8)|i2cReceiveArr[1]);// Сохранение считанного параметра в переменную
				n++;
			}
		}
		else stage=2;
